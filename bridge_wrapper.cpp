...
struct alignas(32) PinnCell32 {
    float param_w;         // [0] 가중치
    float spatial_u;       // [4] U-공간 필드
    float spatial_v;       // [8] V-공간 필드
    float adaptive_gain;   // [12] 적응형 게인
    uint32_t cell_status;  // [16] 상태 MUX
    uint32_t coordinate_id;// [20] 인덱스
    uint64_t padding;      // [24] 캐시라인 대칭 패딩
};
